from django.db import models
from django import forms

# Create your models here.
class Input(models.Model):
    input1 = models.FloatField(
        null=True,
        default=None
    )
    input2 = models.FloatField(
        null=True,
        default=None
    )
    input3 = models.FloatField(
        null=True,
        default=None
    )
    input4 = models.FloatField(
        null=True,
        default=None
    )
    input5 = models.CharField(
        max_length=30,
        choices= [('0.0','N'),
                  ('1.0','Y')],
        default='0.0'
    )
    input6 = models.FloatField(
        null=True,
        default=None
    )
    input7 = models.CharField(
        max_length=30,
        choices= [('0.0','N'),
                  ('1.0','Y')],
        default='0.0'
    )
    input8 = models.FloatField(
        null=True,
        default=None
    )
    input9 = models.FloatField(
        null=True,
        default=None
    )
    input10 = models.FloatField(
        null=True,
        default=None
    )
    input11 = models.FloatField(
        null=True,
        default=None
    )
    input12 = models.CharField(
        max_length=30,
        choices= [('0.0','N'),
                  ('1.0','Y')],
        default='0.0'
    )
    input13 = models.FloatField(
        null=True,
        default=None
    )
    input14 = models.FloatField(
        null=True,
        default=None
    )
    input15 = models.CharField(
        max_length=30,
        choices= [('0.0','F'),
                  ('1.0','M')],
        default='0.0'
    )
    input16 = models.CharField(
        max_length=30,
        choices= [('0.0','N'),
                  ('1.0','Y')],
        default='0.0'
    )
    input17 = models.CharField(
        max_length=30,
        choices= [('0.0','D-pennicillamine'),
                  ('1.0','Placebo')],
        default='0.0'
    )
    input18 = models.CharField(
        max_length=50,
        choices= [('1.0','1'),
                  ('2.0','2'),
                  ('3.0','3'),
                  ('4.0','4')],
        default='2.0'
    )

class InputForm(forms.ModelForm):
    class Meta:
        model = Input    # Typing correct, model without s
        fields = '__all__'
        # Temp using, when Frontend done will have 18 features
        labels = {
            'input1': 'Bilirubin',
            'input2': 'N_Days',
            'input3': 'Prothrombin',
            'input4': 'Copper',
            'input5': 'Edema',              # Add drop-down list already
            'input6': 'Platelets',
            'input7': 'Ascites',            # Add drop-down list already
            'input8': 'Age',
            'input9': 'Albumin',
            'input10': 'SGOT',
            'input11': 'Alk_Phos',
            'input12': 'Spiders',           # Add drop-down list already
            'input13': 'Tryglicerides',
            'input14': 'Cholesterol',
            'input15': 'Sex',             # Add drop-down list already
            'input16': 'Hepatomegaly',    # Add drop-down list already
            'input17': 'Drug',            # Add drop-down list already
            'input18': 'Stage'            # Add drop-down list already
        }

class Output(models.Model):
    output1 = models.CharField(
        max_length=100,
        default='Please SUBMIT Data'
    )